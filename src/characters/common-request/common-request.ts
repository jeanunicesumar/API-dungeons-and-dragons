...
export default class CommonRequest {
  ...
  public async fetchRace(): Promise<Race[]> {
    return (await this.fetchJson<{ results: Race[] }>(`${this.URL}/api/races`))
      .results;
  }
  ...
  public async fetchRaceDetailsByName(name: string): Promise<RaceDetails> {
    return this.fetchJson<RaceDetails>(
      `${this.URL}/api/races/${name.toLocaleLowerCase()}`,
    );
  }

  public async fetchClass(): Promise<Classes[]> {
    const classesData = await this.fetchJson<{ results: Classes[] }>(
      `${this.URL}/api/classes`,
    );
    return classesData.results;
  }

  public async fetchClassDetails(
    randomicClass: Classes,
  ...
  public async fetchClassDetailsByName(name: string): Promise<ClassDetails> {
    return this.fetchJson<ClassDetails>(
      `${this.URL}/api/classes/${name.toLocaleLowerCase()}`,
    );
  }

  public async fetchAbility(): Promise<AbilityScore[]> {
    return (
      await this.fetchJson<{ results: AbilityScore[] }>(
        `${this.URL}/api/ability-scores`,
      )
    ).results;
  }
  ...
  public async fetchFeatures(classes: string, level: number): Promise<any> {
    return this.fetchJson<any>(
      `${this.URL}/api/classes/${classes}/levels/${level}/features`,
    );
  }
}

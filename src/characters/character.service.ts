...
> {
  ...
  public async update(id: string, body: UpdateCharacterDto): Promise<void> {
    ...
    const characterUpdated: Character = this.adapter.updateToEntityConvert(
      body,
      character,
    );
    const createDTO: CreateCharacterDto =
      this.adapter.entityToCreate(characterUpdated);
  ...
  private async buildCharacter(character: Character): Promise<void> {
    const raceDetails: RaceDetails =
      await this.commonRequest.fetchRaceDetailsByName(character.race);
    const classDetails: ClassDetails =
      await this.commonRequest.fetchClassDetailsByName(character.class);

    character.features = await this.getFeatures(
      character.class.toLocaleLowerCase(),
      character.level,
    );
    character.ability = (
      await this.commonRequest.fetchAbilityDetails(
        await this.getRandomAbility(),
      )
    ).full_name;
    character.ability_bonuses = this.mapAbilityBonuses(raceDetails);
    character.alignment = raceDetails.alignment;
    character.proficiencies = classDetails.proficiencies
      ? classDetails.proficiencies.map((proficiency) => proficiency.name)
      : [];
    character.equipment = classDetails.starting_equipment
      ? classDetails.starting_equipment.map((equip) => equip.equipment.name)
      : [];
  ...
  async getRandomSubclass() {
    ...
    const subclass = (await classDetails).subclasses.map(
  ...
  async getFeatures(classes: string, level: number): Promise<any> {
    ...
    for (let currentLevel = level; currentLevel >= 1; currentLevel--) {
      try {
        const fetchFeatures = await this.commonRequest.fetchFeatures(
          classes,
          currentLevel,
        );
        const features = (await fetchFeatures).results.map(
  ...
  mapAbilityBonuses(raceDetails: RaceDetails): any {
    const abilityBonuses: AbilityScoreName[] = raceDetails.ability_bonuses.map(
      (ability) => ({
  ...
  async createRandomCharacter(): Promise<Character> {
    const [randomRace, classResult, randomAbility, subclass, level] =
      await Promise.all([
        this.getRandomRace(),
        this.getRandomClass(),
    ...
    const classDetails =
      await this.commonRequest.fetchClassDetails(classResult);
    const abilityDetails =
      await this.commonRequest.fetchAbilityDetails(randomAbility);
    ...
    const createCharacter: Character = {
      ...
      proficiencies: classDetails.proficiencies
        ? classDetails.proficiencies.map((proficiency) => proficiency.name)
        : [],
      equipment: classDetails.starting_equipment
        ? classDetails.starting_equipment.map((equip) => equip.equipment.name)
        : [],
